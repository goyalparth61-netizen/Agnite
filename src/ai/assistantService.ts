...
function generalDomainAnswer(question:string):string|null{
  ...
  if(/how.*predict|prediction model|future risk|risk estimate/.test(q)) return 'General explanation: AGNITE future-risk windows use the selected hotspot’s thermal history, recurrence/persistence, baseline deviation when available, and supplied context. The trained recurrence path estimates repeat satellite thermal detection, not a guaranteed fire event or exact future fire time.';
...
export function localAnswer(question: string, data: Intelligence): string {
  ...
  else if(/future|predict|next|24|48|7.day|when.*fire|happen next/.test(q)) answer=`${prediction}\n\nAGNITE cannot determine an exact future fire time from this evidence. Thermal-recurrence/risk estimate — not a confirmed future fire prediction.`;
  ...
  else if(/why.*risk|risk.*high/.test(q)) answer=`${prediction}\n\nThese are transparent screening/model factors, not a calibrated probability of a confirmed fire.`;
  ...
  else answer=`${history}\n\n${classification}\n\nEstimated risk windows: ${predictions.map(p=>`${p.window} ${p.riskScore}/100 (${p.level})`).join(', ')||'not available'}. These scores are not operational fire probabilities; any heuristic fallback is an unvalidated simulation. Ask me about history, classification, risk factors, prediction, FRP, FIRMS, persistent heat or precautions.`;
  return `${header}\n\n${answer}\n\n${data.limitations}`;
}
...
export async function askAgnite(question: string, context: Intelligence, conversationOrFetch:ConversationTurn[]|typeof fetch=[], fetchImplArg: typeof fetch = fetch) {
  ...
  try {
    ...
    if(result.mode!=='provider'||typeof result.answer!=='string'||!result.answer.trim()) return fallback('External AI is not configured; answered locally from the selected evidence.');
