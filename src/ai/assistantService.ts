...
export function localAnswer(question: string, data: Intelligence): string {
  ...
  else answer=`${history}\n\n${classification}\n\nEstimated risk windows: ${predictions.map(p=>`${p.window} ${p.riskScore}/100 (${p.level})`).join(', ')||'unavailable'}. These are an unvalidated simulation, not operational fire probabilities. Ask me about history, classification, risk factors, prediction, FRP, FIRMS, persistent heat or precautions.`;
  return `${header}\n\n${answer}\n\n${data.limitations}`;
}
