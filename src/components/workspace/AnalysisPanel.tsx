...
export default function AnalysisPanel({selected,observations,context,setContext,report,run,save,demo}:Props){
 ...
 return <>
  ...
  <div className="ws-two-col"><section className="ws-panel"><div className="ws-panel-head"><h2>Site & context</h2><span className="ws-status">{selected?.source||'NO DATA'}</span></div>
   ...
   </div><p className="ws-source">AGNITE automatically suggests land cover / nearest industrial distance from mapped OpenStreetMap context when available. Treat it as evidence, not verified facility containment; you can override it. Wind remains user supplied.</p>
  ...
  {report&&<>
   ...
   <section className="ws-panel"><h2>Why this result?</h2><p>Feature contributions compare the leading synthetic class with the runner-up. They are model effects, not causal proof.</p>{report.contributions.length?<div className="ws-score-list">{report.contributions.map(item=><div className="ws-score" key={item.feature}><div><span>{item.feature}</span><strong>{item.contribution>0?'+':''}{item.contribution.toFixed(2)}</strong></div><small>{item.direction} leading class</small><div className="ws-score-track"><i style={{width:`${Math.min(100,Math.abs(item.contribution)*20)}%`,background:item.direction==='supports'?'var(--amber)':'var(--ice)'}}/></div></div>)}</div>:<p>No class explanation is emitted when the model abstains.</p>}<details><summary>Risk index method</summary><p>{report.risk.method}</p></details></section></div>
